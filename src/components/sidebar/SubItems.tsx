...
const SubItems: React.FC<SubItemsProps> = ({ parentLabel, subItems, isExpanded, textColor, onItemClick, activeItem }) => {
  const [activeSubItem, setActiveSubItem] = React.useState<string>(() => {
    try {
      ...
      console.error('Error reading from localStorage:', JSON.stringify(error, null, 2));
    }
    return activeItem || '';
  });
          ...
          <SubItemFeatureGuard key={subItem.label} featureName={subItem.label}>
            <ListItem
              ...
              sx={{
                pl: 3,
                cursor: 'pointer',
                backgroundColor: activeSubItem === subItem.label ? 'rgba(52, 211, 153, 0.9)' : 'rgba(204, 217, 255, 0.1)',
                '&:hover': {
                  backgroundColor: activeSubItem === subItem.label ? 'rgba(52, 211, 153, 0.95)' : 'rgba(235, 242, 255, 0.2)',
                  transform: 'translateX(4px)',
                  boxShadow: activeSubItem === subItem.label ? '0 4px 12px rgba(52, 211, 153, 0.25)' : 'none',
                },
                borderRadius: '10px',
                margin: '4px 0',
                transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                boxShadow: activeSubItem === subItem.label ? '0 2px 8px rgba(52, 211, 153, 0.2)' : 'none',
              }}
            ...
            >
              <ListItemIcon
                sx={{
                  minWidth: '30px',
                  color: activeSubItem === subItem.label ? '#fff' : textColor || 'rgba(255, 255, 255, 0.7)',
                }}
              >
                <FiberManualRecordIcon
                  sx={{
                    ...
                    transform: activeSubItem === subItem.label ? 'scale(1.3)' : 'scale(1)',
                  }}
                />
              </ListItemIcon>
              <ListItemText
                ...
                sx={{
                  color: activeSubItem === subItem.label ? '#fff' : textColor || 'rgba(255, 255, 255, 0.9)',
                  '& .MuiTypography-root': {
