...
> = ({
  ...
  const checkStatus = async () => {
    setLoading(true);
    try {
      // First try the local health API endpoint
      try {
        console.log(
          'Checking backend status via local health API endpoint'
        );
        ...
        if (
          ...
          response.data.backend.status ===
            'online'
        ) {
          // Create a BackendStatus object from the health API response
          ...
          // If the backend is offline according to our health API
          throw new Error(
            response.data.backend.error ||
              'Backend is offline'
          );
        }
      } catch (error) {
        // If the health API fails, fall back to direct backend check
        ...
        // Fall back to direct backend check
            ...
            >
              ...
              >
                ...
                <Typography variant="body2">
                  ...
                  using mock data, so you can
                  continue to use it for
                  demonstration purposes. To use
                  real data, please ensure the
