...
const RoleManagement: React.FC = () => {
  ...
  const logRoleChange = (action: 'create' | 'update' | 'delete', roleId: number | null) => {
    ...
    console.log('Role change logged:', JSON.stringify(logData, null, 2));
  };
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <BackendStatusChecker showWhenRunning={false} />
      <Box
        sx={{
          borderBottom: 1,
          borderColor: 'divider',
                ...
                <TableRow key={role.id} hover>
                  ...
                  <TableCell>
                    <Chip
                      ...
                      size="small"
                      color={role.id === selectedRoleId ? 'primary' : 'default'}
                    />
                  </TableCell>
      ...
      <TabPanel value={tabValue} index={1}>
        {selectedRoleId ? (
          ...
          <Typography>Please select a role to manage permissions</Typography>
        )}
      </TabPanel>
      ...
      <TabPanel value={tabValue} index={2}>
        {selectedRoleId ? (
          <Box>
            <Typography variant="h6" gutterBottom>
              Users in {roles?.find((r) => r.id === selectedRoleId)?.name} Role
            </Typography>
            <UserRoleAssignment roleId={selectedRoleId} />
          </Box>
        ) : (
          <Typography>Please select a role to manage users</Typography>
        )}
      </TabPanel>
