...
const PricingPackageCard: React.FC<PricingPackageProps> = memo(({ packageData, onBuyNow }) => {
  ...
  if (packageData.type.toLowerCase().includes('custom') && displayPrice === 0) {
  ...
  return (
    <Card className={`${styles.card} ${isCustom ? styles.custom : ''} ${isDisabled ? styles.disabled : ''}`}>
      ...
      <div className={styles.priceSection}>
        {packageData.testPeriodDays > 0 && <div className={styles.trial}>{packageData.testPeriodDays} days free trial</div>}
      ...
      <CardFooter className={styles.footer}>
        <Button className={`${styles.button} ${isCustom ? styles.contactButton : ''}`} onClick={onBuyNow} disabled={isDisabled}>
