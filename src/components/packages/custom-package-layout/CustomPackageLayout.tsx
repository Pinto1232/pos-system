...
> = ({
  ...
  const [featurePrices, setFeaturePrices] =
    useState<Record<number, number>>({});

  // New state variables for dynamic price calculation
  const [planDiscount, setPlanDiscount] =
    useState<number>(0); // Percentage discount (0-1)
  const [supportPrice, setSupportPrice] =
    useState<number>(0); // Additional price for support
  const [totalPrice, setTotalPrice] =
    useState<number>(basePrice); // Total calculated price
  ...
  useEffect(() => {
    if (features.length > 0) {
      ...
      if (addOns.length > 0) {
        console.log(
          '===== PROCESSING ADD-ONS IN CUSTOM PACKAGE LAYOUT ====='
        );
        console.log(
        ...
        addOns.forEach((addOn, index) => {
          console.log(
            `Processing AddOn #${index + 1} (ID: ${addOn.id}):`
          );
  ...
  useEffect(() => {
    console.log(
      'DEBUG - Price calculation triggered'
    );
    console.log(
      `DEBUG - Current currency: ${selectedCurrency}, Rate: ${rate}`
    );
    console.log(
      `DEBUG - Base price (raw): ${basePrice}`
    );
    console.log(
    ...
    const usageTotal = Object.entries(
      usageQuantities
    ).reduce((sum, [idStr, quantity]) => {
    ...
    const featuresSum =
      featureTotal + addOnTotal + usageTotal;
    console.log(
    ...
    const subtotal =
      basePrice + featuresSum + supportPrice;
  ...
  const handleFeatureToggle = (
    ...
    if (isSelected) {
      ...
      setTotalFeaturePrice(
        (prevTotal) => prevTotal - featurePrice
      );
      console.log(
        `Removed feature ${feature.name} (ID: ${feature.id}), price: ${featurePrice}, new total: ${totalFeaturePrice - featurePrice}`
      );
    } else {
      ...
      console.log(
        `Added feature ${feature.name} (ID: ${feature.id}), price: ${featurePrice}, new total: ${totalFeaturePrice + featurePrice}`
      );
    }
  ...
  const handleAddOnToggle = (addOn: AddOn) => {
    ...
    if (isSelected) {
      ...
      setTotalFeaturePrice(
        (prevTotal) => prevTotal - addOnPrice
      );
      console.log(
  ...
  const handleUsageUpdate = (
    ...
    if (usageItem) {
      ...
      const priceDifference =
        (newValue - oldValue) * usagePrice;

      // Update the total price
      setTotalFeaturePrice(
        (prevTotal) => prevTotal + priceDifference
      );
      console.log(
        `Updated usage ${usageItem.name} (ID: ${id}) from ${oldValue} to ${newValue}, price difference: ${priceDifference}, new total: ${totalFeaturePrice + priceDifference}`
      );
    }
  ...
  const formatPrice = (
        ...
        try {
          ...
          if (
            ...
            displayPrice =
              multiCurrency[currency];
            console.log(
              `DEBUG - Using multiCurrency price: ${displayPrice}`
            );
          } else {
    ...
    console.log(
      `DEBUG - Final formatted price: ${result}`
    );
    return result;
  };
  ...
  const handlePlanSelect = (index: number) => {
    ...
    const planDiscounts = [
      ...
      0.1, // Quarterly Plan - 10% discount
      0.15, // Biannual Plan - 15% discount
      0.2, // Annual Plan - 20% discount
    ...
    if (selectedPlanIndex === index) {
      ...
      setPlanDiscount(0);
      console.log(
        `Payment plan deselected. Discount reset to 0%`
      );
    } else {
      // If selecting a new plan, apply the corresponding discount
      setSelectedPlanIndex(index);
      const newDiscount =
        index < planDiscounts.length
          ? planDiscounts[index]
          : 0;
      ...
      console.log(
        `Discount applied: ${newDiscount * 100}%`
      );
      console.log(
  ...
  const handleSupportSelect = (index: number) => {
    ...
    if (selectedSupportIndex === index) {
      ...
      const baseForSupport =
        basePrice + totalFeaturePrice;
      const multiplier =
        index < supportMultipliers.length
          ? supportMultipliers[index]
          : 0;
      const newSupportPrice =
        baseForSupport * multiplier;
                        ...
                        onClick={() => {
                          ...
                          if (
                            ...
                            const packageData = {
                              ...
                              usageQuantities,
                              calculatedPrice:
                                totalPrice, // Use our dynamically calculated price
                              selectedCurrency,
                              planDiscount,
                              supportLevel:
                                selectedSupportIndex,
