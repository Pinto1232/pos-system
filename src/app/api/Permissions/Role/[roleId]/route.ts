...
export async function GET(request: Request, context: { params: { roleId: string } }) {
  ...
  try {
    ...
    if (!useMockData) {
      ...
      const response = await fetch(`${BACKEND_API_URL}/api/Permissions/Role/${roleId}`, {
        method: 'GET',
        headers: {
...
export async function PUT(request: Request, context: { params: { roleId: string } }) {
  const params = await context.params;
  const roleId = params.roleId;
  ...
  try {
    ...
    console.error('Error updating role permissions:', JSON.stringify(error, null, 2));
    return NextResponse.json(
      {
