...
export async function POST(
  ...
  try {
    console.log(`Proxying POST request to backend to enable package ${packageId} for user: ${userId}`);

    const response = await fetch(`${BACKEND_API_URL}/api/UserSubscription/user/${userId}/enable-package/${packageId}`, {
      method: 'POST',
      headers: {
    ...
    console.log('Successfully enabled package in backend');
