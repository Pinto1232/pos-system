...
// Server action to fetch pricing packages
export async function fetchPricingPackagesAction(
  ...
  try {
    ...
    const apiUrl = `${baseUrl}/api/pricing-packages?pageNumber=1&pageSize=10${refresh ? '&refresh=true' : ''}`;

    // Get auth token from cookies - this is server-only code
    ...
    // Fetch the data with optimized caching configuration
    ...
    const data: PricingPackagesResponse =
      await response.json();

    // Process and return the packages
...
// Define a type for the package data from API
type PackageData = {
  id?: number | string;
  title?: string;
  description?: string;
...
// Helper function (not exported, so doesn't need to be async)
function createPackageFromData(
  ...
  const validType = packageTypes.includes(type)
    ? type
    : 'starter-plus';
...
// Function to sort packages in the correct order
