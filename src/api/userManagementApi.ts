...
import { User, Role, Permission, UserFormData, RoleFormData } from '@/types/userManagement';

const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5107/api';
...
api.interceptors.request.use(
  (config) => {
    ...
    const token = localStorage.getItem('accessToken') || 'dev-token-for-testing';
    ...
    if (process.env.NODE_ENV === 'development') {
      config.headers['X-Dev-Override-Auth'] = 'true';
    }
...
export const updateUser = async (id: number, userData: UserFormData): Promise<void> => {
  await api.put(`/api/users/${id}`, { ...userData, id });
};
...
export const assignRolesToUser = async (userId: number, roles: string[]): Promise<void> => {
  await api.post(`/api/users/${userId}/roles`, roles);
};

export const getUserRoles = async (userId: number): Promise<string[]> => {
  const response = await api.get<string[]>(`/api/users/${userId}/roles`);
  return response.data;
};

export const getUserPermissions = async (userId: number): Promise<string[]> => {
  const response = await api.get<string[]>(`/api/users/${userId}/permissions`);
  return response.data;
};
...
export const updateRole = async (id: number, roleData: RoleFormData): Promise<void> => {
  await api.put(`/api/roles/${id}`, { ...roleData, id });
};
...
export const assignPermissionsToRole = async (roleId: number, permissions: string[]): Promise<void> => {
...
export const fetchPermissionsByModule = async (module: string): Promise<Permission[]> => {
  const response = await api.get<Permission[]>(`/api/permissions/module/${module}`);
  return response.data;
};

export const fetchModules = async (): Promise<string[]> => {
  const response = await api.get<string[]>('/api/permissions/modules');
  return response.data;
};
