...
const mockRolePermissions: Record<number, string[]> = {
  ...
  2: [ // Manager
    ...
    'reports.view', 'reports.create', 'transactions.approve',
    'inventory.view', 'inventory.edit',
  ],
  3: [ // Cashier
    'sales.create', 'products.view', 'customers.view', 'customers.create',
  ],
  4: [ // Inventory Manager
...
const permissionService = {
    ...
    try {
      ...
      if (authzClient) {
        try {
          ...
          console.error('Error fetching permissions from Keycloak:', keycloakError);
  ...
  getRolePermissionsByName: async (roleName: string): Promise<string[]> => {
    try {
      ...
      console.error(`Error fetching permissions for role with name ${roleName}:`, error);
  ...
  updateRolePermissions: async (roleId: number, permissions: string[]): Promise<void> => {
    try {
      ...
      console.error(`Error updating permissions for role with ID ${roleId}:`, error);
      throw error;
    }
  },
