...
import { fetchAvailableCurrencies, Currency } from '@/api/currencyApi';
...
const fetchCustomization = async (
  ...
  try {
    ...
    const response = await fetch(`/api/UserCustomization/${userId}`);
...
> = ({
  ...
  onCustomizationUpdated,
    ...
    const [regionalSettings, setRegionalSettings] =
      useState<RegionalSettings>(DEFAULT_REGIONAL_SETTINGS);
    ...
    useEffect(() => {
      ...
      if (data) {
        ...
        if (data.regionalSettings) {
          ...
          setRegionalSettings(data.regionalSettings);
        } else {
          console.log('Using default regional settings');
    ...
    const updateCustomizationMutation =
      useUpdateCustomization<UserCustomization, UserCustomization>();
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  <Typography>Enable Multi-Currency Support</Typography>
                  <Button
                ...
                {regionalSettings.enableMultiCurrency && (
                  <Box sx={{ mt: 2 }}>
                    ...
                    >
                      {availableCurrencies.length > 0 ? (
                        availableCurrencies.map((currency) => (
                          <Button
                            key={currency.code}
                            variant={regionalSettings.supportedCurrencies.includes(currency.code) ? "contained" : "outlined"}
                            color={regionalSettings.supportedCurrencies.includes(currency.code) ? "primary" : "inherit"}
                            size="small"
                            onClick={() => {
                              const updatedCurrencies = regionalSettings.supportedCurrencies.includes(currency.code)
                                ? regionalSettings.supportedCurrencies.filter(c => c !== currency.code)
                                : [...regionalSettings.supportedCurrencies, currency.code];
                        ...
                        ['ZAR', 'USD', 'EUR', 'GBP', 'AUD', 'CAD', 'JPY', 'CNY'].map((currency) => (
                          <Button
                            key={currency}
                            variant={regionalSettings.supportedCurrencies.includes(currency) ? "contained" : "outlined"}
                            color={regionalSettings.supportedCurrencies.includes(currency) ? "primary" : "inherit"}
                            size="small"
                            onClick={() => {
                              const updatedCurrencies = regionalSettings.supportedCurrencies.includes(currency)
                                ? regionalSettings.supportedCurrencies.filter(c => c !== currency)
                                : [...regionalSettings.supportedCurrencies, currency];
                    ...
                    <Typography
                      ...
                      color="text.secondary"
                    >
                      Select the currencies you want to support in your POS system. The default currency will always be supported.
                    </Typography>
                  </Box>
                )}
                ...
                <Box sx={{ mt: 3 }}>
                  ...
                  >
                    <MenuItem value="#,###.##">
                      1,234.56 (Comma as thousand separator)
                    </MenuItem>
                    <MenuItem value="#.###,##">
                      1.234,56 (Period as thousand separator)
                    </MenuItem>
                    <MenuItem value="# ###.##">
              ...
              >
                ...
                >
                  <Typography>Auto-detect Location</Typography>
                  <Button
              ...
              >
                ...
                <Typography
                  ...
                  sx={{ mb: 1 }}
                >
                  These settings control how dates, times, numbers, and currencies are displayed throughout your POS system.
                </Typography>
                <Typography
                  variant="body2"
                  color="text.secondary"
                  sx={{ mb: 1 }}
                >
                  Changes to these settings will be applied immediately across all parts of the application.
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
                ...
                >
                  Default Currency: {regionalSettings.defaultCurrency}
                </Typography>
              </Box>
            </Box>
          );
